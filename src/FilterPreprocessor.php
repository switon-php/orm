...
class FilterPreprocessor implements FilterPreprocessorInterface
{
    ...
    protected function convertDateRange(string $entityClass, string $_field, mixed $min, mixed $max): array
    {
        ...
        if ($min !== null && $min !== '' && is_string($min) && !str_contains($min, ':')) {
            if (is_numeric($min)) {
                $min = (int)$min;
            } else {
                $timestamp = strtotime($min . ' 00:00:00');
                $min = $timestamp !== false ? (int)$timestamp : $min;
            }
        }
        if ($max !== null && $max !== '' && is_string($max) && !str_contains($max, ':')) {
            if (is_numeric($max)) {
                $max = (int)$max;
            } else {
                $timestamp = strtotime($max . ' 23:59:59');
                $max = $timestamp !== false ? (int)$timestamp : $max;
            }
        }
