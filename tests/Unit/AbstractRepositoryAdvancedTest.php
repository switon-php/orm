...
class AbstractRepositoryAdvancedTest extends TestCase
{
    ...
    public function testValuesPassesPreprocessedFiltersOrderByAndValuesFieldToQuery(): void
    {
        $raw = ['status' => 1];
        $normalized = ['status' => 1, 'values_ready' => true];
        $field = 'name';

        $mockFp = $this->createMock(FilterPreprocessorInterface::class);
        $mockFp->expects($this->once())
            ->method('preprocess')
            ->with($raw, TestEntity::class)
            ->willReturn($normalized);

        $repo = new TestableAdvancedRepository(
            $this->mockEntityMetadata,
            $this->mockRelationManager,
            $this->mockEntityManager,
            $this->mockQueryBuilder,
    ...
    public function testExistsPassesPreprocessedFiltersToQuery(): void
    {
        $raw = ['status' => 1];
        $normalized = ['status' => 1, 'normalized' => true];
        ...
        $this->mockQuery->expects($this->once())
            ...
            ->with($normalized)
            ->willReturnSelf();
        $this->mockQuery->method('exists')->willReturn(true);

        $this->assertTrue($repo->exists($raw));
    }

    public function testExistsByIdPassesPreprocessedPrimaryKeyFilterToQuery(): void
    {
        $raw = ['id' => 7];
        $normalized = ['id' => 7, 'from_preprocessor' => true];
        ...
        $this->assertTrue($repo->existsById(7));
    }

    public function testCountPassesPreprocessedFiltersToQuery(): void
    {
        $raw = ['status' => 2];
        $normalized = ['status' => 2, 'count_ready' => true];
        ...
        $this->mockQuery->expects($this->once())
            ...
            ->with($normalized)
            ->willReturnSelf();
        $this->mockQuery->method('count')->willReturn(3);

        $this->assertSame(3, $repo->count($raw));
    }
    ...
    public function testPluckSkipsRowsWhenKeyColumnMissingFromRow(): void
    {
        $rows = [
            ['id' => 1, 'name' => 'A'],
            ['name' => 'B'],
        ...
        $this->mockQuery->method('execute')->willReturn($rows);

        $result = $this->repository->pluck(['status' => 1], 'name', 'id');

        $this->assertSame([1 => 'A', 3 => 'C'], $result);
    }

    public function testPluckMapsMissingValueFieldToNull(): void
    {
        $rows = [
            ['id' => 1],
            ['id' => 2, 'name' => 'B'],
        ...
        $result = $this->repository->pluck([], 'name', 'id');

        $this->assertSame([1 => null, 2 => 'B'], $result);
    }

    public function testPluckSelectUsesExplicitKeyAndValueFieldNames(): void
    {
        $rows = [
            ['code' => 'x', 'title' => 'T'],
        ];
        ...
        $result = $this->repository->pluck([], 'title', 'code');

        $this->assertSame(['x' => 'T'], $result);
    }
    ...
    public function testWherePassesPreprocessedFiltersToQuery(): void
    {
        $raw = ['flag' => 1];
        $normalized = ['flag' => 1, 'via_where' => true];
