...
final class TestHydratorJsonPayload implements JsonSerializable
{
...
class TestHydratorJsonEntity extends Entity
{
    #[Id]
    public int $id;

    public ?TestHydratorJsonPayload $payload = null;
}

final class TestHydratorStringableLabel implements Stringable
{
...
class TestHydratorStringableEntity extends Entity
{
    #[Id]
    public int $id;

    public ?TestHydratorStringableLabel $label = null;
}
...
class EntityHydratorTest extends TestCase
{
    ...
    public function testHydrateArrayFieldThrowsWhenValueIsNeitherArrayNorJsonString(): void
    {
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('expected JSON array payload');
    ...
    public function testDehydrateEncodesJsonSerializableField(): void
    {
        $entity = new TestHydratorJsonEntity();
        $entity->id = 1;
        $entity->payload = new TestHydratorJsonPayload('alpha');

        $data = $this->entityHydrator->dehydrate($entity);

        $this->assertSame('{"label":"alpha"}', $data['payload']);
    }

    public function testDehydrateCastsStringableFieldToString(): void
    {
        $entity = new TestHydratorStringableEntity();
        $entity->id = 1;
        $entity->label = new TestHydratorStringableLabel('scoped');

        $data = $this->entityHydrator->dehydrate($entity);

        $this->assertSame('scoped', $data['label']);
    }

    public function testHydrateHydratableReusesExistingInstance(): void
    {
        $existing = new TestHydratableValueObject('seed');
        ...
        $this->assertSame($existing, $entity->payload);
    }
}
